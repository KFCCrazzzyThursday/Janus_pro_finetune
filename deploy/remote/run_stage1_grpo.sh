...
export JANUS_GRPO_DATASET_NUM_PROC="${JANUS_GRPO_DATASET_NUM_PROC:-8}"
export JANUS_GRPO_DATALOADER_WORKERS="${JANUS_GRPO_DATALOADER_WORKERS:-4}"
...
export JANUS_LOCAL_ROLLOUT_FORWARD_BATCH_SIZE="${JANUS_LOCAL_ROLLOUT_FORWARD_BATCH_SIZE:-16}"
...
mkdir -p \
  "${TMPDIR}" "${XDG_CACHE_HOME}" "${MODELSCOPE_CACHE}" "${TORCH_HOME}" \
  "${JANUS_STAGE1_GRPO_OUTPUT}"
