...
OUTPUT_DIR="${ROOT_DIR}/outputs/stage1/scienceqa_sft"
SMOKE=0

if [[ "${1:-}" == "--smoke" ]]; then
  SMOKE=1
  shift
  OUTPUT_DIR="${ROOT_DIR}/outputs/smoke/stage1_scienceqa_sft"
fi
...
# All checkpoint/data/output paths are either NFS or tmpfs. No persistent local
# disk is used. Unset proxies for any incidental dependency or hub check.
...
COMMON_ARGS=(
  ...
  --check_model false
  --dataset "${ROOT_DIR}/data/processed/scienceqa/train_sft.jsonl"
...
# FSDP2 keeps full-parameter GaLore while sharding parameters, gradients, and
# optimizer states across the four 46 GiB cards. DeepSpeed remains an opt-in
# for hosts that provide CUDA_HOME/nvcc; plain DDP is retained for diagnostics.
...
echo "Launching stage-1 ScienceQA SFT on physical GPUs ${CUDA_VISIBLE_DEVICES}."
